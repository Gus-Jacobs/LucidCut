...
export default function ResultsPanel({ results, previewUrl, onSeek }: Props) {
  ...
  return (
    <div className="card" style={{ height: '100%', display: 'flex', flexDirection: 'column', gap: 18 }}>
      <h2 style={{ margin: 0, fontSize: '1.8em', fontWeight: 800 }}>🎯 Detected Words</h2>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 16, minHeight: 0, overflow: 'auto' }}>
        {matched.length === 0 ? (
          <div className="muted" style={{ fontSize: '1.1em', padding: '40px 20px', textAlign: 'center' }}>No matching words detected. ✨</div>
        ) : (
